...
# ---------- 机器人行为（可选，均可在管理面板里改）----------
# USDT 对人民币汇率（用于在界面上显示 CNY 等价金额）
#USDT_RATE=7.20
# 允许使用机器人管理命令的 TG 用户名（不含 @，逗号分隔）
...
| 导航项 | 路径 | 说明 |
|--------|------|------|
| 📊 首页 | `/admin` | 数据总览（用户、会员、收入、套餐、订单管理） |
| ⚙️ 机器人配置 | `/admin/bot-config` | Bot Token、汇率、欢迎语、超时、到期提醒等 |
...
- **套餐管理**：
  ...
  - 一键停售/恢复
- **会员管理**：按 Telegram 用户 ID 搜索、调整到期时间、移除+踢出群
...
**付款后跳转地址**（`return_url`）：用户支付完成后跳回的页面地址，可选填，留空则不跳转。虎皮椒和易支付的该字段独立配置。

### 6.6 安全提醒

- 登录有频率限制（每 IP 每分钟最多 5 次），防止暴力破解
- 密码使用 PBKDF2-HMAC-SHA256（10 万轮）哈希存储，不存明文
- 首次登录强制修改默认密码，未修改前无法使用任何管理功能
- 密码最少 8 位，修改时需输入当前密码验证
- 所有表单带 CSRF token 防护
- HTTP 下密码明文传输——生产环境务必配 Nginx + HTTPS（见第 8 节）
...
| `/active` | 所有活跃订阅列表 |
| `/broadcast <消息>` | 群发所有人（自动限速约 25 条/秒，避免触发电报限流） |
...
- **每小时**检查到期会员 → 自动踢出群（同时解禁，允许再次付费加入）
- **每天 0 点**检查即将到期的会员 → 提前 N 天（在"机器人配置"页可设置，默认 3 天）发续费提醒；设为 0 即禁用。每条订阅在一个提醒周期内只发一次，续费后重置。
- **每 15 分钟**检查超时未支付订单 → 自动标记过期并通知用户重新下单
- **支付成功回调**：自动验证签名 → 创建或延长订阅 → 发送入群链接；签名校验失败的回调返回 400 拒收
- **重复/迟付保护**：并发回调或超时后迟到的合法付款，系统用原子操作防止重复履约，迟付款自动补发会员并在后台标记"迟付复活"
- **重复下单清理**：用户点同一个套餐的支付方式时，旧的未支付订单自动取消
- 入群需申请，非订阅者自动拒绝
- 已停售套餐被老客户点击时提示"不再可用"，不会生成订单
