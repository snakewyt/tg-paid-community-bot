...
| 导航项 | 路径 | 说明 |
|--------|------|------|
| 📊 首页 | `/admin` | 数据总览（用户、会员、收入、套餐、订单） |
| 👥 会员管理 | `/admin/members` | 赠送、搜索、导出、优惠链接 |
...
- **收入统计**：按支付渠道分组的累计收入
- **机器人已加入的群组 / 频道**：自动列出机器人当前所在的群/频道（名称、类型、用户名、chat_id、是否为管理员）。把机器人加入群组或设为管理员后，这里会实时更新——不必再手动去查 chat_id
- **套餐管理**：
...
- **免费体验邀请链接**：创建后得到 Telegram 邀请链接；用户点链接申请入群 → 自动获得 N 天会员。后台可随时改 N（只影响之后新入群用户）。可设使用次数上限、链接过期时间，可撤销链接。创建时可选择适用对象：**全部用户 / 仅新用户 / 仅老用户**（新用户=该群从未有过会员记录，老用户=曾有过）
- **付费折扣链接**：生成 `https://t.me/Bot?start=promo_xxx`；用户点开后购买对应套餐按折扣（百分比优先，或固定减免）计费。改折扣只影响之后新订单。同样可限制仅新用户或仅老用户
