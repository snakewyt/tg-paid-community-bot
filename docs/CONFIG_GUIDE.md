...
- **套餐管理**：
  ...
  - 一键停售/恢复
- **会员管理**：按 Telegram 用户 ID 搜索、调整到期时间（`+7` / `-3` / `YYYY-MM-DD`，改到过去会**立即踢人**）、移除+踢出群
- **赠送会员**：填写用户 ID、选择套餐、天数，点击赠送；系统自动履约并发送入群链接（与 `/grant` 命令相同逻辑）
...
| 管理员 TG 用户名 | 逗号分隔，不含 `@`（**仅备注**，Bot 管理员权限认 `ADMIN_IDS`） | — |
...
**付款后跳转地址**（`return_url`）：用户支付完成后跳回的页面地址，可选填，留空则不跳转。
...
| `/plans` | 查看所有套餐 |
| `/stats` | 收入统计 + 活跃会员数 |
| `/grant <用户ID> <套餐ID> <天数>` | 赠送会员（与后台「赠送会员」相同） |
| `/setexpiry <用户ID> <套餐ID> <+N/-N/日期>` | 调整到期（改到过去**立即踢人**） |
| `/active` | 活跃订阅列表（最多显示 100 条） |
| `/broadcast <消息>` | 群发（限速 ~25 条/秒，单次最多 5000 人） |

> Bot 管理员**仅** `.env` 中 `ADMIN_IDS` 的数字 ID 有效，后台填写的 TG 用户名不授予 Bot 命令权限。
...
- **延期**：后台会员管理填 `+7` / `-3`，或 `/setexpiry 用户ID 套餐ID +7`
- **赠送**：后台「赠送会员」表单，或 `/grant 用户ID 套餐ID 天数`
