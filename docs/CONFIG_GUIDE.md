...
- **活跃会员列表**：搜索、调整到期（`+7` / `-3` / `YYYY-MM-DD`）、移除并踢出；点击用户名/ID 进入**用户详情页**（`/admin/members/{用户ID}`）。套餐列：付费/赠送显示套餐名，免费体验显示**活动名称**；同一用户多条有效订阅在同一行叠加显示
- **用户详情**：基本信息、全部订阅（套餐/群组/状态/到期/开通/支付方式/金额/订单）、订单历史（只读；调整仍在列表页操作）
- **导出 CSV**：导出当前活跃会员（含用户名、套餐/体验活动名、群、到期、关联订单渠道/金额），UTF-8 BOM，可用 Excel 打开
- **免费体验 / 优惠码**：创建时**自动随机生成**优惠码；用户私聊机器人发送优惠码即可开通体验天数，并收到入群链接。同时仍会生成 Telegram 邀请链接（点链接申请入群也可开通）。后台可随时改天数（只影响之后新兑换/入群用户）。可设使用次数上限、链接过期时间，可**删除**整条活动（同时尽量撤销 Telegram 邀请链接）。创建时可选择适用对象：**全部用户 / 仅新用户 / 仅老用户**（新用户=该群从未有过会员记录，老用户=曾有过）
...
- **机器人命令赠送**：仍可用 `/grant <用户ID或@用户名> <套餐ID> <天数>` 赠送会员（后台入口已移除）
